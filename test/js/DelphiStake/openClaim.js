...
contract('DelphiStake', (accounts) => {
  describe('Function: openClaim', () => {
    ...
    it('should not allow the arbiter to be whitelisted', async () => {
      try {
        await ds.whitelistClaimant(arbiter, arbiter, conf.minFee, conf.deadline, "", { from: staker });
      ...
      assert(false, 'Expected whitelisting of arbiter to fail');
    });

    it('should not allow the staker to whitelist themselves', async () => {
      try {
        await ds.whitelistClaimant(staker, arbiter, conf.minFee, conf.deadline, "", { from: staker });
      ...
      assert(false, 'expected whitelisting of staker to fail');
    });

    it('should not allow a whitelisted individual to open a claim after their deadline', async () => {
      await ds.whitelistClaimant(other, arbiter, conf.minFee, '1', "", { from: staker });

      await token.approve(ds.address, feeAmount, { from: other });

      try {
        await ds.openClaim(1, claimAmount, feeAmount, '', { from: other });
      } catch (err) {
        assert(utils.isEVMRevert(err), err.toString());
    ...
    it('should not allow a non-whitelisted individual to open a claim', async () => {
      ...
      try {
        await ds.openClaim(0, claimAmount, feeAmount, '', { from: other });
      } catch (err) {
        assert(utils.isEVMRevert(err), err.toString());
    ...
    it('should revert if _fee is smaller than the minimum', async () => {
      ...
      try {
        await ds.openClaim(0, claimAmount, conf.minFee - 1, '', { from: claimant });
      } catch (err) {
        assert(utils.isEVMRevert(err), err.toString());
    ...
    it('should revert if _amount + _fee is greater than the available stake', async () => {
      ...
      try {
        await ds.openClaim(0, claimAmount, amount, '', { from: claimant });
      } catch (err) {
        assert(utils.isEVMRevert(err), err.toString());
    ...
    it('should add a new claim to the claims array and properly initialize its properties',
      async () => {
        ...
        await ds.openClaim(0, claimAmount, feeAmount, 'newclaim', { from: claimant });
        ...
        assert.strictEqual(claim[3].toString(10), feeAmount, 'initialized claim fee incorrectly');
    ...
    it('should emit a NewClaim event', async () => {
      ...
      await ds.openClaim(0, claimAmount, feeAmount, '', { from: claimant }).then((status) => {
    ...
    it('should append claims to the end of the claim array, without overwriting earlier claims', async () => {
      await token.approve(ds.address, feeAmount, { from: claimant });
      await ds.openClaim(0, claimAmount, feeAmount, 'claim1', { from: claimant });

      await token.approve(ds.address, feeAmount, { from: claimant });
      await ds.openClaim(0, claimAmount, feeAmount, 'claim2', { from: claimant });

      await token.approve(ds.address, feeAmount, { from: claimant });
      await ds.openClaim(0, claimAmount, feeAmount, 'claim3', { from: claimant });
      ...
      assert.strictEqual(claim1[3].toString(10), feeAmount, 'initialized claim fee incorrectly');
      ...
      assert.strictEqual(claim2[3].toString(10), feeAmount, 'initialized claim fee incorrectly');
      ...
      assert.strictEqual(claim3[3].toString(10), feeAmount, 'initialized claim fee incorrectly');
